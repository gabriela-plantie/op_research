...
class AbstractGraph(ABC):
    ...
    def _check_node(self, node):
        if not self.contains_node(node):
            raise ValueError("Nodes should be added first")
        return None
...
class UndirectedGraph(AbstractGraph):
    ...
    def render(self, engine=None):
        ...
        edges = [
            (n1, n2, dist)
            for (n1, adjacents) in self._distances.items()
            for (n2, dist) in adjacents.items()
        ]
...
def dijkstra(graph, origin, destiny):
    ...
    while (visited != destiny) and (visited is not None):
        ...
        # update distances
        if len(adjacents) != 0:
            [distances, previous] = _update_distances(
                graph, adjacents, visited, distance_to_visiting, distances, previous
            )
