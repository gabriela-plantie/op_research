...
class Optimal:
    def __init__(self, all_elementos, capacity):
        ...
        self.tabla = [
            [Bag(c, []) for _ in range(len(all_elementos) + 1)] for c in range(capacity + 1)
        ]
    ...
    def evaluate_adding_element(self, hasta_capacidad, hasta_elemento, element):
        ...
        if valor_replace > 0 and valor_replace >= valor_keep:
...
class Bag:
    ...
    def __repr__(self):
        return str(
            f"capacidad: {self.capacidad}, peso: {self.suma_pesos()}, valor: {self.suma_valor()}, elegidos: {self.elementos_elegidos}"
        )


all_elementos = [Elemento(1, 4, 5), Elemento(2, 5, 6), Elemento(3, 2, 3)]

capacity = 9

# one option choose by value//weight, 
# si capacity es un numero chico en relacion a los valores de los elementos no va a ser optimo.
